...
Creation is **thread-safe**: a lock guarantees concurrent threads never produce duplicate instances for the same key.
...
Create or retrieve a globally shared singleton instance. Creation is thread-safe, guarded by a lock. The cache key is the combination of the class, positional args, and keyword args.
