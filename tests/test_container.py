...
class ContainerTest:
    ...
    def test_overrides_do_not_leak_between_threads(self):
        import threading
        from concurrent.futures import ThreadPoolExecutor

        container = Container(DatabaseFactory())
        mock_database = object()
        inside_override = threading.Event()
        thread_two_finished = threading.Event()
        ...
        def thread_one() -> None:
            with container.override(Database, mock_database):
                ...
                inside_override.set()
                thread_two_finished.wait(timeout=5)

        def thread_two() -> DatabaseService:
            inside_override.wait(timeout=5)
            try:
                return container.provide(DatabaseService)
            finally:
                thread_two_finished.set()

        with ThreadPoolExecutor(max_workers=2) as pool:
            future_one = pool.submit(thread_one)
            future_two = pool.submit(thread_two)
            service_from_thread_two = future_two.result()
            future_one.result()

        assert service_from_thread_two.dependency is not mock_database
...
class RecursiveResolutionTest:
    def test_factory_that_provides_its_own_return_type_raises_error(self):
        ...
        with pytest.raises(
            RecursiveResolutionError, match="Recursive resolution detected for Service"
        ):
            container.provide(Service)
